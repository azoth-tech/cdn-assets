...
CADDY_SITE_TEMPLATE="" ; CADDY_DOMAIN="" ; CADDY_AGENTS_DOMAIN="" ; CADDY_SITES_DIR="/opt/caddy/sites" ; CADDY_CONTAINER="caddy" ; CORS_ALLOWED_ORIGINS_REGEX=""
...
while IFS= read -r line || [[ -n "$line" ]]; do
  ...
  if [[ "$key" == ENV.* ]]; then
    ...
    case "$key" in
      ...
      CADDY_AGENTS_DOMAIN)  CADDY_AGENTS_DOMAIN="$value" ;;
      CADDY_SITES_DIR)      CADDY_SITES_DIR="$value" ;;
      CADDY_CONTAINER)       CADDY_CONTAINER="$value" ;;
      CORS_ALLOWED_ORIGINS_REGEX) CORS_ALLOWED_ORIGINS_REGEX="$value" ;;
...
RESOLVE_VARS=(SSH_HOST SSH_PORT SSH_USER SSH_KEY REMOTE_DIR IMAGE_REPO DOPPLER_PROJECT DOPPLER_CONFIG INIT_POSTGRES_DSN DOCKER_NETWORK CADDY_SITE_TEMPLATE CADDY_DOMAIN CADDY_AGENTS_DOMAIN CADDY_SITES_DIR CADDY_CONTAINER CORS_ALLOWED_ORIGINS_REGEX HEALTHCHECK_TYPE HEALTHCHECK_TARGET COMPOSE_FILE_LOCAL UPLOAD_FILES ENSURE_DIRS)
if [[ ${#DOPPLER_KEY_NAMES[@]} -gt 0 ]]; then
...
# ---- upload extra files (BEFORE Caddy reload, so uploaded certs etc. ----
# are in place by the time caddy reload imports their site snippet).
...
if [[ -n "$CADDY_SITE_TEMPLATE" && -f "$CADDY_SITE_TEMPLATE" ]]; then
  ...
  SITE_FILE="$(mktemp 2>/dev/null || mktemp -t site-snippet)"
  {
    ...
    tpl="$(sed -e "s|{{DOMAIN}}|$CADDY_DOMAIN|g" -e "s|{{CADDY_DOMAIN}}|$CADDY_DOMAIN|g" -e "s|{{CADDY_AGENTS_DOMAIN}}|$CADDY_AGENTS_DOMAIN|g" "$CADDY_SITE_TEMPLATE")"
    if [[ -n "${CORS_ALLOWED_ORIGINS_REGEX:-}" ]]; then
      tpl="${tpl//\{\{CORS_ALLOWED_ORIGINS_REGEX\}\}/$CORS_ALLOWED_ORIGINS_REGEX}"
    fi
    printf '%s\n' "$tpl"
  } > "$SITE_FILE"
