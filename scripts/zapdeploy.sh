...
CADDY_SITE_TEMPLATE="" ; CADDY_DOMAIN="" ; CADDY_SITES_DIR="/opt/caddy/sites" ; CADDY_CONTAINER="caddy" ; CORS_ALLOWED_ORIGINS_REGEX=""
...
if [[ -n "$CADDY_SITE_TEMPLATE" && -f "$CADDY_SITE_TEMPLATE" ]]; then
  ...
  SITE_FILE="$(mktemp 2>/dev/null || mktemp -t site-snippet)"
  {
    ...
    tpl="$(sed -e "s|{{DOMAIN}}|$CADDY_DOMAIN|g" -e "s|{{CADDY_DOMAIN}}|$CADDY_DOMAIN|g" "$CADDY_SITE_TEMPLATE")"
